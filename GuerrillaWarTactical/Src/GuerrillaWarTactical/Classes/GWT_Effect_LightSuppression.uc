class GWT_Effect_LightSuppression extends X2Effect_Persistent config(GWT_Stats_Effects);

var config int Soldier_LightSuppression_AimPenalty;     //  inside GetToHitModifiers, this value is used if the Attacker is not on eTeam_XCom (because a soldier hit this unit with suppression)
var config int Alien_LightSuppression_AimPenalty;       //  as above, but only for eTeam_XCom (because an alien hit this xcom unit with suppression)
var config int Multiplayer_LightSuppression_AimPenalty; //  the value used in MP games
...
function GetToHitModifiers(XComGameState_Effect EffectState, XComGameState_Unit Attacker, XComGameState_Unit Target, XComGameState_Ability AbilityState, class<X2AbilityToHitCalc> ToHitType, bool bMelee, bool bFlanking, bool bIndirectFire, out array<ShotModifierInfo> ShotModifiers)
{
	...
	if (!bIndirectFire)
	{
		...
		if (`XENGINE.IsMultiplayerGame())
			ShotMod.Value = default.Multiplayer_LightSuppression_AimPenalty;
		else if (Attacker.GetTeam() != eTeam_XCom)
			ShotMod.Value = default.Soldier_LightSuppression_AimPenalty;
		else
			ShotMod.Value = default.Alien_LightSuppression_AimPenalty;
